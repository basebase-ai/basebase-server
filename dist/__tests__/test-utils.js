...
class TestHelper {
    ...
    async setup() {
        ...
        try {
            ...
            this.mongoClient = new mongodb_1.MongoClient(this.mongoUri);
            ...
            if (typeof serverModule.initializeForTesting === "function") {
                ...
                throw new Error("initializeForTesting function not found in server module");
            }
        }
        catch (error) {
    ...
    async createTestUser() {
        ...
        try {
            ...
            const verifyResponse = await (0, supertest_1.default)(this.app).post("/verifyCode").send({
                phone: this.testPhoneNumber,
                code: actualCode,
    ...
    authenticatedRequest(token) {
        return {
            get: (url) => (0, supertest_1.default)(this.app).get(url).set("Authorization", `Bearer ${token}`),
            post: (url) => (0, supertest_1.default)(this.app).post(url).set("Authorization", `Bearer ${token}`),
            put: (url) => (0, supertest_1.default)(this.app).put(url).set("Authorization", `Bearer ${token}`),
            patch: (url) => (0, supertest_1.default)(this.app).patch(url).set("Authorization", `Bearer ${token}`),
            delete: (url) => (0, supertest_1.default)(this.app).delete(url).set("Authorization", `Bearer ${token}`),
        };
    }
}
